...
# 翻译走本地模型服务：TRANSLATION_API_URL=http://<host>:<port>/translate
# 不配这一项服务照常跑，来信不再统一翻译，非英文邮件的情绪与关键词规则会退化
python server.py
```
...
**历史记录**
一行 = 一个 thread：用户邮件 + AI 回信 + **当时的订单上下文快照**（`context_json`）+ **当时的政策判定**（`policy_json`）+ **当时的邮件标签**（`tags_json`）+ **当时的英文基准译文**（`translation_json`：译文、原文语种、语种是谁判的、有没有真的调翻译服务、失败原因）。
...
**生成回复分三步**（`translation.py` → `tags.py` → `prompts.py`）
```
收到邮件 → ⓪ translation.to_english()  统一翻译成英文基准，同时定下原文语种
         → ① tags.analyse()  读懂这封信：意图 / 情绪 / 紧急度 / 语言 / 关键实体
         → ② prompts + llm  把标签连同订单事实和政策判定一起交给写信的模型
```
第一步的提示词放在 `email_automatic_reply_en_US.jinjia2` 而不是 Python 里，按 `# User Prompt` 这行切成 system / user 两半，用 jinja2 渲染 `buyer_message` + 可选的 `product_context` / `order_context` + 可选的 `translation_note`。改分类体系只动模板，不动代码。

**第零步：所有分析在英文上做，回信在客户的语言上做**（`translation.py`）
这条链路上除了写回信的大模型，其余每个环节都只懂英文：本地情绪模型只覆盖英文（非英文输入返回 `AMBIGUOUS` + 9 个簇全 0）；人工复核的关键词表（`lawyer` / `chargeback` / `injured`…）是英文字面量，一封写着 "voy a llamar a mi abogado" 的信一条规则都命中不了；分类模板本身也是英文写的。所以把翻译提到链路最前面，让后面所有判断都在同一种语言上做——**译文是给模型的，不是给人的**。实测那封西语催退款的信，翻成英文后 `lawyer` 关键词规则正常命中，情绪模型也给出了 `P0_ESCALATE` 而不是"没有判断"。

`tags.analyse()` 收到的 `subject` / `body` 就是这份英文文本，于是 `language` 标签不能再由大模型判（它读的是译文，只会回答 `en`）：语种由翻译服务的 fasttext 确定性写入 `language`，大模型那份存进 `model_language` 留档——和 `sentiment` / `needs_review` 一样的两路留档规则。模板里那段 `translation_note` 会告诉分类模型"你读到的是机翻"，免得它把机翻的生硬措辞当成买家表达不清，凭空抬高 `ambiguity_level`。

**回信不是把英文回信机翻回去**，而是让大模型直接用客户的语种写——机翻回去的信读起来就是机翻，而这是整条链路上客户唯一真正会看到的东西。提示词里同时给原文和译文，并写明**以原文为准**，译文只是"给模型的一副眼镜"。语种不在服务支持的 9 种（en / es / de / fr / it / pt / zh / ja / ko）之内时回信退回英文：模型写得出来是一回事，我们能不能对这封信的质量负责是另一回事。

两个已知的服务端行为，代码专门照顾了：
- **英文进英文出不是恒等变换**（实测 "it still has not shipped" 会被改写成 "it still hasn't been shipped"），而一次调用在 CPU 上要十几到三十秒。所以本地先用 `is_english_baseline()` 判一次，确认本来就是英文就跳过调用（`TRANSLATION_SKIP_ENGLISH=0` 关掉）。这个本地判别刻意保守：出现任何非拉丁文字、别的语种的专属字符、或者英文功能词没有明显占优，都算"不确定"，宁可多花一次调用——误判成"是英文"的代价是下游模型读到一封外语邮件，比多等十几秒贵得多。
- **超出 9 种支持语言时 `detected_primary_language` 会报成 `en`**（实测一封俄语来信译文完全正确，语种字段却是 `en`）。所以本地判别还兼一个职责：认出西里尔 / 阿拉伯 / 泰文这类脚本时，语种记 `other`、回信退回英文，而不是让一封俄语信被当成英语信。

`to_english()` 和 `analyse()` 一样**永不抛异常**：服务挂了就退回原文 + 本地正则判语种，链路照常跑完，失败原因记进 `translation.error` 并在阅读窗格上明说"下面的分析是在原文上跑的、会退化"。主题和正文用**一次**调用翻完——主题作为第 0 段拼在正文前面发过去，服务端按空行切段且段落数进出一致，`paragraphs[0]` 就是主题的译文；段落数对不上时主题保留原文，宁可主题不译也不要把正文第一段错当成主题。
...
- **模型给的值要过白名单。** `INTENTS` / `URGENCIES` 照抄模板里的表，`SENTIMENTS` 引用情绪模型的 9 个簇，`LANGUAGES` 引用翻译服务支持的 9 种语言 + `other`，模型编出来的意图会被压成 `other`，越界的置信度直接丢弃，不进数据库。
...
- 语言：`REPLY LANGUAGE` 由翻译层判定（fasttext，不是模型的猜测）直接指定，要求写地道母语而非英译外——各语种的敬语形式在提示词里逐一点名（西语 usted、德语 Sie、法语 vous、日语敬語、韩语 존댓말）；订单号 / 运单号 / 商品名保持原样不译；非支持语种回英语。`prompts.detect_language()` 只剩人工回信和前端提示这类"不值得为它调一次翻译服务"的路径在用。
...
接上翻译层之后，情绪模型拿到的一直是英文基准文本，这条兜底路径只在**翻译服务本身不可用**（或 `TRANSLATION_API_URL` 没配）时才会走到——但它必须留着：那正是它存在的意义。情绪测试台（下一节）不走翻译，那里测的是模型本身，把非英文输入悄悄翻成英文等于把体温计调到 36.5 度。
...
**翻译模型测试台**（`GET /?view=translation`）
和情绪测试台同构的第二块体检板，rail 的第五项。跑的模型是 **`tencent/Hy-MT2-1.8B`**（腾讯混元，2026-05-21 开源，Apache-2.0，1.8B dense；同系列还有 7B 和 30B-A3B MoE）。它不是 OPUS-MT / NLLB 那种 encoder-decoder 专用翻译网络，而是一个**因果语言模型**，翻译任务靠提示词下达——好处是听得懂"术语表""保持结构"这类指令，代价是官方推荐解码参数里 `temperature=0.7`，同一封邮件翻两次措辞可能不同。标签读的是译文，所以这是整条链路上唯一一处"同样的输入不保证同样的输出"。语种判别是另一个模型（fasttext lid.176），译文和语种是两条独立的链路，**译文对、语种错是最常见的一种组合**。

五段结构——
- **模型**：顶部是一张 hero——型号、许可证、发布方，端点此刻的状态做成一枚会呼吸的绿点摆在型号旁边，下面四块 stat tile 给出**来的人真正想知道的四个数**（参数量 / 开放 9 种·模型 33 种 / 上下文 / 一次调用的实测区间）。这四个数原先埋在一张八行的 label-value 表里——一个要在表里找的数字，等于页面没有告诉你。往下依次是**「此刻真正加载的是什么」**（`GET /api/translation/meta` 会去问服务自己的 `/health` 和 `/languages`，把 `model_path`、设备、fasttext 是否就绪、开放了哪几种语言显示出来；说明书上写死一个模型名没有意义，真正该回答的是"这个端点后面现在跑的是什么"，服务开放的语种和 `SUPPORTED_LANGS` 对不上时直接标 drift）、剩下的规格（架构 / 解码参数 / 上下文）、同系列三个尺寸（**我们在跑的那一行整行高亮**，因为那是这张表唯一会被问的问题），最后是**两张分开的卡**：「厂商公布的结论」和「在这台机器上的实际代价」。分成两张卡而不是一张卡的两段，是为了让一条 benchmark 战绩永远没有机会挨着一个我们实测的延迟、被读成同一种事实。
  耗时表的 Wall time 一列在数字下面带一根条（单序列、单色、数值仍是文字，条只负责让形状一眼可见）：输入长 27 倍不等于等 27 倍，而最短的那条 11 个字照样要 3.5 秒。这一列**左对齐**而不是数字右对齐——数字必须压在条的起点上，两者才读成一个记号。
- **链路**：一封邮件从进到出的九步，九个序号被一条竖轨串起来（不然九行读起来是九个互不相干的方块，而它们是一条路），每步标明归属（我们 / 服务 / 模型），上方带一行图例说明这三个词各指什么。这张图要说明两件事：**只有第 6 步是模型**（整行高亮、序号填成实心），其余八步里有四步是我们自己的代码——所以这里能出的问题大多不是权重的问题。后面接响应契约表（字段名照抄不译，那是你在服务端日志里会 grep 的键）。
- **已知行为**：契约里没写、但实测确实会发生的四件事，每条都配一条能一键复现的用例（下详）。
- **试一段**：任意文本直接喂服务（上限 600 字），或点 12 条内置用例之一。用例**分成两组**——「日常来信」（开放的每种语言各一条）和「故意会翻车的三条」（各对应一条已知行为，卡片带琥珀色左边条）；12 张几乎一样的卡片铺成一个网格就是一堵墙，分组之后点哪一条才是有意图的选择。分组写在后端的 `kind` 字段里，不在前端按 id 硬编。每条用例的语种码放在提示行最前面，扫一列时先看见的应该是"这是哪国话"。没有载入结果时下面接的是**「最近跑过的」**而不是半屏空白——这是块公开板子，第一件有用的事就是别人刚试了什么。
  **测试台不走跳过英文那道闸门**——那是生产链路的优化，测试台测的是模型本身，把英文输入悄悄跳过去等于只展示模型顺利的那一面；闸门的判定照样算出来单独显示。结果卡把**三方对语种的说法并排摆出来**（服务端 fasttext / 本地正则 / 仲裁结果，标出谁赢了），紧接着一条**结论横幅**直接说清这次是"三方一致"还是"服务端被本地改判了"——语种判错和译文翻错要用完全不同的方式修，得先分清是哪一种，而不是让人自己去比对上面那三格。再往下是逐片明细表，某一片的语种和整封判定对不上、或者压根没拿到标签的那一行会被标出来，那正是回信语种被带偏的地方。
- **统计与记录**：测试次数 / 译文好评率 / 语种准确率 / 平均耗时，链路判定的语种分布表，以及**判定 × 人认为的真实语种**混淆表。

**评价分两轴，因为这条链路有两个独立的产出。** `verdict`（`good` / `bad`）说的是译文，`true_lang` 说的是语种，两轴各自可以只填一个，统计时分母也各算各的。合成一个"对/错"会把最有价值的那类记录抹掉：一封俄语信可以译文完全正确、而语种被报成 `en`——两轴在这条记录上的方向是相反的。语种选择器**预填链路自己的判定**，同意只要点保存，不同意才要改，所以混淆表上每一个偏离对角线的格子都是有人真的动手改过的。

四条已知行为都是对着我们自己的端点实测的（2026-08-03，CPU 单机），各配一条内置用例：
| 行为 | 实测证据 | 用例 |
|---|---|---|
| 英文进英文出不是恒等变换 | `"it still has not shipped"` → `"it hasn't been shipped yet"` | `en_passthrough` |
| 超出开放语种时报成 `en` | 俄语来信译文完全正确，`detected_primary_language=en`，逐片标签为空 | `ru_unsupported` |
| 短片的语种标签不可靠 | 主题 `"Pedido roto"`（11 字）判成 `pt`，同一封信的正文判成 `es`，整封跟着报 `pt` | `short_subject` |
| CPU 上是秒级不是毫秒级 | 301 字 / 2 段 = 35.4s | `de_long_email` |

第三条尤其值得注意：**主题行自成一段**，它上面一个错标签就可能带偏整封信，而回信语种跟的正是整封那个结论。第二条则是本地脚本判别兜底的存在理由——服务报 `en` 但文本明摆着是西里尔字母时，仲裁听本地的，语种记 `other`、回信退回英文（`resolve_source_lang()`，`to_english()` 和测试台共用同一个函数，免得两份实现分叉之后测试台展示的是一条不存在的链路）。

耗时表里最该看的不是绝对值而是形状：**耗时跟着生成的 token 数走，不跟输入长度走**——11 个字的主题照样要 3.5 秒。这正是生产链路在来信本来就是英文时直接跳过整次调用的原因。

视图和单条测试同样进 URL（`?view=translation&tab=bench&tx=7`）。
...
| GET | `/api/translation/meta` | 模型档案 + 九步链路 + 响应契约 + 四条已知行为 + 语种词表 + 12 条用例 + **服务实时探针** |
| POST | `/api/translation/test` | `{text, sample_id?}` → 跑一次翻译并落库（不走英文闸门；失败也落库） |
| GET | `/api/translation/tests?scope=all\|mine` | 所有人的测试记录（含逐片明细） |
| GET | `/api/translation/tests/{id}` | 单条测试，给 `?tx=` 分享链接用 |
| POST | `/api/translation/tests/{id}/feedback` | `{verdict, true_lang, note}` → 两轴人工评价，三者全空则 400 |
| GET | `/api/translation/stats?scope=all\|mine` | 译文好评率、语种准确率（分母各算各的）、语种分布、判定 × 真实语种混淆表 |
| GET | `/api/health` | 模型名、base_url、key 是否加载成功、翻译层是否配置与支持语种 |

## 前端

无构建，原生 JS。左侧 rail 顶部切换五个工作区：**邮箱**、**政策评估**、**标签分析**、**情绪识别**、**翻译**。

后两个是"本地模型说明书 + 测试台"，共用同一套组件（CSS 里那批 `emo-` 前缀的类是**两个页面共享的组件库**，前缀是历史遗留不是作用域，`styles.css` 里有注释说明）。两块面板各用各的 class（`.emoview` / `.txview`）而不是共一个，理由和当初 `.emoview` 不复用 `.tagview` 一样：共用一个 class 会让切换视图时两块面板同时显示。两个页共用 `?tab=` 参数（它们互斥），深链的记录 id 各用各的（`?test=` / `?tx=`）。
...
**英文基准卡**排在分析卡前面（同一个骨架，边色由紫换蓝：紫色是"模型读出来的结论"，蓝色是"模型读到的原料"）。只有真的翻译过才渲染——原文本来就是英文时同一段文字登两遍不是信息。摆出来是因为分析卡上的每一个判断读的都是这份译文而不是客户的原文，不摆出来就没法核对"模型到底读到了什么"。翻译失败时反过来一定要渲染：那正是"分析在原文上跑的、会退化"这件事该被看见的时候。
...
- **语言不画图**，直接列带标签的计数——一个只有几片的饼图不比数字更清楚。词表是翻译服务支持的 9 种加一个 `other`，但真实分布通常集中在两三种上。
...
翻译边界按"这段文字是谁写的"来划：
- **界面文案 + 枚举值 + 场景名翻译**——UI 文案（`I18N`）、邮件标签（`TAG_LABELS`：12 意图 / 9 情绪簇 + 2 个已退役的旧情绪值 / 4 紧急度 / 9 语言 + `other`）、场景标题与简介（`scenarios.TRANSLATIONS`，随 `/api/scenarios` 下发到 `i18n` 字段）都是三语。场景名出现在 rail 筛选、写信卡片、政策台导航和邮件列表四处，统一走 `scenarioText()`；历史邮件若对应场景已不存在，回退到发信时落库的标题。
